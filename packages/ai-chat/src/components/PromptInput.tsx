...
export function PromptInput({
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      ...
      {attachedFile && (
        <div className={`flex items-center justify-between gap-3 self-start rounded-2xl border border-zinc-200/50 dark:border-zinc-800/40 bg-white/90 dark:bg-zinc-900/80 backdrop-blur-md shadow-sm transition-all duration-300 animate-in fade-in slide-in-from-bottom-2 duration-300 ${
          ...
          <div className="flex items-center gap-2">
            <svg className="w-4 h-4 text-zinc-500 dark:text-zinc-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-[9px] text-zinc-400 dark:text-zinc-550 font-mono truncate">{attachedFile.type || "binary"} • {formatSize(attachedFile.size)}</span>
            </div>
          </div>
          <button 
            onClick={removeFile}
            className="rounded-full p-1 text-zinc-400 hover:text-zinc-850 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
          >
      ...
      <div className={`relative flex w-full items-end rounded-2xl border border-zinc-200/50 dark:border-zinc-800/40 bg-white/80 dark:bg-zinc-900/40 backdrop-blur-md shadow-sm transition-all duration-300 vfx-input-halo ${
        ...
        <button
          ...
          disabled={disabled}
          type="button"
          className={`mb-0.5 flex shrink-0 items-center justify-center rounded-xl text-zinc-400 hover:text-zinc-800 dark:hover:text-zinc-200 hover:bg-zinc-100/50 dark:hover:bg-zinc-800/40 transition-all disabled:opacity-40 ${
